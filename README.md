...
- ```extract_patches.py``` serves the purposes for both training and validation data generation.
- To generate validation data, you could extract a portion of the training numpy array that is generated by ```extract_patches.py```. Not much edits in ```train.py``` and ```extract_patches.py``` are needed.
----
